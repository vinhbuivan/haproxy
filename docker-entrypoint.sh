...
function clean_up {
  ...
  echo "net.ipv4.ip_nonlocal_bind = 1" | grep "net.ipv4.ip_nonlocal_bind\s*=\s*0" > /dev/null || [ "$INITIAL_IP_NONLOCAL_SETTING" -eq "1" ] || echo "Switching off ipv4.ip_nonlocal_bind" && echo "0" > /var/proc/sys/net/ipv4/ip_nonlocal_bind
...
function set_up {
  echo "net.ip_nonlocal_bind = 0" | grep "net.ipv4.ip_nonlocal_bind\s*=\s*1" > /dev/null || echo "Setting ipv4.ip_nonlocal_bind" && INITIAL_IP_NONLOCAL_SETTING="0" && echo "1" > /var/proc/sys/net/ipv4/ip_nonlocal_bind
